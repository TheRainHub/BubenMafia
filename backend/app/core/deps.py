...
from app.crud.users import \
    get_user_by_id  # вам понадобится CRUD‑функция для юзеров
from app.db import get_session
from app.models.user import \
    User  # поправьте путь, если у вас модель лежит в другом месте
...
def current_gm(
    ...
    if not user or user.role != "gm":
        raise HTTPException(
            ...
            detail="Только GM может выполнять эту операцию",
        )
    return user
